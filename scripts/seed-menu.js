  ...
  for (const category of categories) {
    try {
      ...
      if (result.status === 201 || result.status === 200) {
        ...
        const getResult = await makeRequest('GET', `http://localhost:${PORT}/api/v1/menu/categories`);
  ...
  for (const item of dineInItems) {
    ...
    try {
      const itemData = {
        ...
        descriptionNl: dutchDescriptions[item.name] || item.description,
        category: categoryId,
        menuType: 'dine-in',
